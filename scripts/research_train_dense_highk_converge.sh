...
if [[ -z "${SLURM_JOB_ID:-}" ]]; then
  echo "ERROR: scripts/research_train_dense_highk_converge.sh must run inside a Slurm allocation." >&2
  exit 2
fi
...
run_one() {
  ...
  "${PYTHON_BIN}" verify_mwm_data.py --paper-parity
  "${PYTHON_BIN}" train_mwm.py "${config}"
}
...
case "${ENV_NAME}" in
  ...
  all)
    ...
    ;;
  *)
    echo "Usage: scripts/research_train_dense_highk_converge.sh {pusht|tworoom|all}" >&2
    exit 2
    ;;
esac
