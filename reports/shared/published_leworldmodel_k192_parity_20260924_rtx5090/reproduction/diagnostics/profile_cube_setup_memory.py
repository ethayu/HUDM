"""Diagnostic of unused memory after exact Cube runtime setup on a selected device.
...
parser.add_argument('--label', choices=('default_seeded', 'malloc_seeded', 'cuda_default_seeded', 'cuda_malloc_seeded'), required=True)
...
def sample(label):
    ...
    if torch.cuda.is_initialized():
        item['cuda_allocated_bytes'] = torch.cuda.memory_allocated()
        item['cuda_reserved_bytes'] = torch.cuda.memory_reserved()
    samples.append(item)
...
def rng_fingerprint():
    cuda_rng = [state.cpu().numpy().tobytes() for state in torch.cuda.get_rng_state_all()] if torch.cuda.is_initialized() else []
    return hashlib.sha256(pickle.dumps((random.getstate(), np.random.get_state(),
                                      torch.get_rng_state().numpy().tobytes(), cuda_rng))).hexdigest()
...
runtime = load_eval_runtime(str(config), overrides=[f'device={args.device}'])
assert runtime.device.type == args.device
if args.device == 'cpu':
    assert not torch.cuda.is_initialized(), 'The CPU diagnostic must remain CPU-only'
else:
    assert torch.cuda.is_initialized()
sample('runtime_loaded_before_fingerprint')
...
receipt = {'diagnostic_only': True, 'completed_at': datetime.datetime.now(datetime.timezone.utc).isoformat(),
           'device': args.device, 'diagnostic_seed': 42, 'variant': args.label,
           ...
           'model_parameter_bytes': sum(t.numel() * t.element_size() for t in runtime.model.parameters()),
           ...
           'scope': 'Model/data setup memory only; no environment construction or rollouts. Does not prove the full 50-environment experiment fits.'}
