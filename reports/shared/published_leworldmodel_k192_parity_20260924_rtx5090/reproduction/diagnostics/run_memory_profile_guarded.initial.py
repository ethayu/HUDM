...
for process in psutil.process_iter(['pid', 'cmdline']):
    ...
    if str(ROOT) in ' '.join(args) and (str(ROOT / 'run_local.py') in args or 'mwm.benchmark.matrix' in args):
...
command = [str(ROOT / 'venv/bin/python'), '-u', str(ROOT / 'profile_cube_setup_memory.py')]
log_path = ROOT / 'cube_allocator_profile.log'
...
with log_path.open('a') as output:
    ...
    output.flush()
    child = subprocess.Popen(command, cwd=ROOT / 'HUDM', stdout=output, stderr=subprocess.STDOUT)
...
receipt = {'diagnostic_only': True, 'started_at': started_at,
           ...
           'command': command, 'returncode': returncode, 'guard_stop_reason': reason,
...
(ROOT / 'cube_allocator_profile_guard_receipt.json').write_text(json.dumps(receipt, indent=2) + '\n')
