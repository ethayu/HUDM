...
def rollout_episode(
    ...
    for t in range(T):
        if policy == 'ou':
            ...
            if is_goal_directed and t > 20:  # Start goal-directed after a few steps
...
def save_zarr(
    ...
    # Create arrays
    g_data.array('img', data=frames.astype(np.float32), shape=(min(100000, frames.shape[0]),) + frames.shape[1:])
    g_data.array('action', data=actions.astype(np.float32), shape=(min(100000, actions.shape[0]), actions.shape[1]))
    g_meta.array('episode_ends', data=ends.astype(np.int64), shape=(max(1, len(ends)),))
    g_data.array('state', data=states.astype(np.float32), shape=(min(160, states.shape[0]), states.shape[1]))
    # minimal attrs
    root.attrs.update({})
