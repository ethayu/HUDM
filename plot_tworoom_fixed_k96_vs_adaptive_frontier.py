"""Build rollouts/tworoom_goal25_fixed_k96_vs_adaptive_frontier.png: does
simply fixing K=96 for the whole plan (no fidelity transitions at all) beat
the best adaptive/annealing schedule at matched compute, or does adaptive
scheduling still win somewhere on the frontier?

Two Pareto frontiers, same env/manifest/horizon/episodes (TwoRoom, goal25,
horizon=2, 100 episodes, dense checkpoint mwm_paper10_tworoom_k96_120_144_168_192):

- "Fixed K=96" frontier: release20260728_tworoom_goal25_densek96_fixed_cem_grid
...
- "Adaptive schedules" frontier: pooled Pareto frontier across all 26
  changing-fidelity dense schedules x the same 25 pop/n_iter combos, from
  release20260728_dense_tworoom_all_fidelity_schedules (650 rows) -- same
  pooling method as plot_pusht_release20260728_horizon5_all_configs.py's
  "winning adaptive schedules" frontier.
"""
...
def make_plot(out_path: str) -> None:
    fixed_rows = load_rows(Path("reports/research/release20260728_tworoom_goal25_densek96_fixed_cem_grid"))
    fixed_x = [r["bits_used_total"] / r["episodes"] / 1e6 for r in fixed_rows]
    fixed_y = [r["success_rate"] for r in fixed_rows]

    adaptive_rows = load_rows(Path("reports/research/release20260728_dense_tworoom_all_fidelity_schedules"))
    ...
    ax.step(afx, afy, where="post", color=ADAPTIVE_COLOR, linewidth=3.0, zorder=3,
            label="Adaptive schedules (pooled Pareto frontier, 26 schedules x 25 CEM settings)")
    ...
    plt.close(fig)
    print(f"Saved {out}")
    print("fixed K=96 frontier:", list(zip([round(x,1) for x in ffx], ffy)))
    print("adaptive frontier:", list(zip([round(x,1) for x in afx], afy)))


if __name__ == "__main__":
    make_plot(out_path="rollouts/tworoom_goal25_fixed_k96_vs_adaptive_frontier.png")
