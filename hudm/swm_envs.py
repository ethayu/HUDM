...
from hudm.swm_restore import restore_spec_for_env
from hudm.swm_wrappers import ogbench_restore_wrapper
...
def parse_image_shape(value: int | str | tuple[int, int] | list[int]) -> tuple[int, int]:
    ...
    if out[0] != out[1]:
        raise ValueError(f"HUDM v1 requires square image_shape, got {out}")
    if out[0] <= 0:
...
def validate_continuous_box_action_space(action_space: Any, env_id: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    if spaces is not None and not isinstance(action_space, spaces.Box):
        raise ValueError(f"HUDM v1 only supports continuous Box actions; {env_id} has {action_space}.")
    ...
    if not (np.all(np.isfinite(low)) and np.all(np.isfinite(high))):
        raise ValueError(f"HUDM v1 requires finite action bounds for {env_id}.")
    return low, high
