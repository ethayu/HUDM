...
class GTEnvCEMPlanner:
    """
    CEM planner that propagates dynamics via direct PushT env rollouts.

    Planning/CEM/fidelity orchestration is shared in `SharedCEMCore`; this
    class implements backend-specific rollout and objective logic.
    """
    ...
    def __init__(
        ...
        self.progress_leave = bool(self.gt_env_cfg.get("progress_leave", False))
        self.env_fidelity_cfg = SharedCEMCore.as_cfg_dict(
            self.gt_env_cfg.get("fidelity_env", {}), "gt_env.fidelity_env"
        )

        if self.rollout_samples <= 0:
            raise ValueError(f"gt_env.rollout_samples must be > 0, got {self.rollout_samples}")
        if self.objective_space not in {"image", "state"}:
            raise ValueError(
                f"gt_env.objective_space must be 'image' or 'state', got {self.objective_space}"
            )
        if self.metric not in {"l1", "l2"}:
            raise ValueError(f"Unsupported objective latent_metric '{self.metric}'. Use 'l1' or 'l2'.")

        # Configure env-level fidelity machinery once.
        if hasattr(self.env, "configure_planning_fidelity"):
            self.env.configure_planning_fidelity(
                enabled=self.core.fidelity_enabled,
                num_levels=self.core.num_levels,
    ...
    def _image_distance(self, img: np.ndarray, goal_img: np.ndarray) -> float:
        if img.shape != goal_img.shape:
            raise ValueError(
                f"Image shape mismatch in gt_env objective: img={img.shape}, goal={goal_img.shape}"
            )
        diff = img - goal_img
        if self.metric == "l1":
    ...
    def _goal_visuals_by_level(
        ...
        for li in unique_levels:
            ...
            goal_obs, _ = self.env.prepare(seed=seed, init_state=goal_state, goal_state=goal_state)
            visual = goal_obs.get("visual", None)
            if visual is None:
                raise ValueError("gt_env backend requires env observations to include 'visual'.")
    ...
    def _candidate_rollout(
        ...
        rollout_levels: List[int],  # len H
        seed: int,
        goal_visual_by_level: Dict[int, np.ndarray],
    ) -> tuple[float, int]:
        obs, state = self.env.prepare(seed=seed, init_state=init_state, goal_state=goal_state)
        del obs
        ...
        for t in range(self.horizon):
            ...
            if hasattr(self.env, "set_planning_fidelity_level"):
                self.env.set_planning_fidelity_level(level_idx)
