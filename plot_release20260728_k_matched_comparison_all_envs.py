"""Build rollouts/release20260728_goal25_k_matched_comparison_all_envs.png:
a 2x2 grid, one subplot per release20260728 env (PushT, Reacher, TwoRoom,
OGB Cube), each showing individually-trained-single-K vs. dense-matched-K
success rate across K in {96,120,144,168,192}, at each env's fixed
paper-default CEM setting (configs/research/release20260728_<env>_goal25_k_matched_comparison.yaml).

goal_offset=25 is used because it is the only offset with a completed
k_matched_comparison report for all four envs as of this writing (reacher
and tworoom goal50 sweeps have not been run).
...
# Brand-neutral, colorblind-safe categorical palette, one hue per env.
ENV_COLOR = {
    "pusht": "#2a5f9e",
    "reacher": "#c9622a",
    "tworoom": "#2a9e6e",
...
def make_plot(goal_offset: int, out_path: str) -> None:
    fig, axes = plt.subplots(1, 4, figsize=(19.0, 4.8), sharex=True, sharey=True)

    for (env_key, env_title), ax in zip(ENVS, axes.flat):
        output_dir = Path(f"reports/research/release20260728_{env_key}_goal{goal_offset}_k_matched_comparison")
    ...
    fig.suptitle(
        f"Individually-trained vs. dense-matched-K  (goal_offset={goal_offset}, 100 episodes)\n"
        f"solid+circle = single-K training, dashed+square = multi-K (dense) checkpoint sliced at K",
...
if __name__ == "__main__":
    make_plot(goal_offset=25, out_path="rollouts/release20260728_goal25_k_matched_comparison_all_envs.png")
