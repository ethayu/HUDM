"""Run a guarded Cube setup diagnostic only after scientific finals finish."""
...
variant = ('cuda_' if args.device == 'cuda' else '') + args.allocator + '_seeded'
...
command = [str(ROOT / 'venv/bin/python'), '-u', str(ROOT / 'profile_cube_setup_memory.py'), '--label', variant, '--device', args.device]
...
receipt = {'diagnostic_only': True, 'started_at': started_at,
           ...
           'command': command, 'diagnostic_device': args.device, 'allocator_environment': allocator_env,
