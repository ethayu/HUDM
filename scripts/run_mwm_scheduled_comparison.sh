...
if [[ -z "${SLURM_JOB_ID:-}" ]]; then
  echo "ERROR: scripts/run_mwm_scheduled_comparison.sh must run inside a Slurm allocation. Submit scripts/slurm_mwm_scheduled_comparison.sbatch with sbatch." >&2
  exit 2
fi
