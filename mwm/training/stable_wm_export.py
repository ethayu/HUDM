...
def load_stable_wm_adapter_lightning_state(model: torch.nn.Module, checkpoint_path: str | Path) -> dict[str, Any]:
    ...
    if missing or unexpected:
        raise ValueError(
            f"Could not load Stable-WM adapter state from {checkpoint_path}: "
            f"missing={list(missing)}, unexpected={list(unexpected)}"
        )
    return checkpoint if isinstance(checkpoint, dict) else {}


def export_stable_wm_adapter_lightning_checkpoint(
    cfg_path: str,
    checkpoint_path: str,
    ...
    try:
        ...
        save_world_checkpoint(
            model,
            run_dir,
            metadata={**metadata_for_stable_wm_adapter_model(metadata, model), **train_info},
        )
    finally:
        close_dataset_handles(base_ds)
    print(f"Exported Stable-WM adapter Lightning checkpoint to canonical MWM checkpoint: {run_dir}")


__all__ = ["load_stable_wm_adapter_lightning_state", "export_stable_wm_adapter_lightning_checkpoint"]
