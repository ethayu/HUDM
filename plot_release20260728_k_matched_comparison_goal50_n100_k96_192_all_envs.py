"""Build rollouts/release20260728_goal50_k_matched_comparison_n100_k96_192_all_envs.png:
...
vs. dense-matched-K), but at goal_offset=50 with 100 episodes (the original
sample size) instead of goal_offset=25/n=200. Uses each env's existing
release20260728_<env>_goal50_k_matched_comparison report, which runs at
horizon=4/receding_horizon=4 (goal50's standard "harder" horizon, matching
release20260728_dense_<env>_goal50_plan100_execute20_all_fidelity_schedules),
not horizon=2 or horizon=5.
...
def make_plot(out_path: str) -> None:
    ...
    for (env_key, env_title), ax in zip(ENVS, axes.flat):
        output_dir = Path(f"reports/research/release20260728_{env_key}_goal50_k_matched_comparison")
    ...
    fig.suptitle(
        "Individually-trained vs. dense-matched-K  (goal_offset=50, horizon=4, 100 episodes)\n"
...
if __name__ == "__main__":
    make_plot(out_path="rollouts/release20260728_goal50_k_matched_comparison_n100_k96_192_all_envs.png")
