...
def _media_html(payload: dict[str, Any], episode_index: int, base_dir: Path) -> str:
    ...
    for kind, entry in sorted(entries.items()):
        ...
        description = (
            "Simulator replay from the stored action trace. Check task completion, contacts, collisions, and whether the success label matches the motion."
            if kind == "env"
            else "Dataset observation (left) versus decoded latent reconstruction (right). Check missing objects, geometry drift, blur, and fidelity changes; this is not a planner prediction."
        )
        cards.append(
            f"<figure data-media-kind='{html.escape(str(kind))}'>"
            f"<video controls preload='auto' src='/{html.escape(href)}'></video>"
            f"<figcaption><strong>{html.escape(label)}</strong><span>{html.escape(description)}</span></figcaption>"
            "</figure>"
        )
...
def rollout_page_html(root: Path, eval_path: Path, episode_index: int) -> str:
    ...
    both_supported = env_supported and latent_supported
    supported_ready = [ready for supported, ready in ((env_supported, env_ready), (latent_supported, latent_ready)) if supported]
    both_ready = both_supported and bool(supported_ready) and all(supported_ready)
    ...
    buttons = "".join(
        [
            ...
            _render_button("both", "Both", supported=both_supported, ready=both_ready),
        ]
    )
...
<head>
  ...
  <style>
    ...
    .guide {{ display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 14px; }}
...
<main>
  ...
  <section class="panel">
    ...
    <p class="muted render-help">Existing videos load directly. Environment replay reads only this rollout's dataset rows and does not load the model; latent reconstruction loads the checkpoint once, then reuses it. GPU acceleration mainly helps latent reconstruction. Re-render only when the renderer or checkpoint changed.</p>
