...
PUSHT_JOB="$(sbatch --parsable scripts/slurm_mwm_train_pusht_dense.sbatch)"
TWOROOM_JOB="$(sbatch --parsable scripts/slurm_mwm_train_tworoom_dense.sbatch)"
BENCHMARK_JOB="$(sbatch --parsable --dependency="afterok:${PUSHT_JOB}:${TWOROOM_JOB}" scripts/slurm_mwm_dense_comparison.sbatch)"
