...
if [[ -z "${SLURM_JOB_ID:-}" ]]; then
  echo "ERROR: scripts/slurm/run_mwm_train_scheduled_env.sh must run inside a Slurm allocation. Submit one of the scripts/slurm/slurm_mwm_train_*_scheduled.sbatch files with sbatch." >&2
  exit 2
fi
