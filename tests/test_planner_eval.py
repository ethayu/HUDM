...
class ExperimentConfigTests(unittest.TestCase):
    ...
    def test_select_rollouts_is_deterministic(self):
        ...
        self.assertEqual(spec.baseline, "finest_fixed")
    ...
    def test_particle_fidelity_sweep_resolves_fixed_and_linear_variants(self):
        spec = resolve_experiment_spec(os.path.join(ROOT, "configs/planner_eval_particle_fidelity_sweep_5.yaml"))
        self.assertEqual(
            spec.variant_names(),
            [
                "particle_l0_n1_fixed",
                "particle_l1_n2_fixed",
                "particle_l2_n3_fixed",
                "particle_l3_n4_fixed",
                "particle_l4_n16_fixed",
                "particle_l5_n64_fixed",
                "particle_l6_n128_fixed",
                "particle_l7_n252_fixed",
                "particle_linear_mpc",
                "particle_linear_mpc_cem",
                "particle_linear_all",
            ],
        )
    ...
    def test_experiment_config_accepts_particle_device_slots(self):
        ...
        backend_cfg = os.path.join(ROOT, "configs/backend/particle_state.yaml")
...
"""
        ...
        try:
            ...
            self.assertEqual(spec.execution["particle"]["device_slots"], ["cuda:0", "cuda:1"])
        finally:
            os.unlink(tmp_path)
...
"""
        ...
        try:
            spec = resolve_experiment_spec(tmp_path)
            self.assertIsNone(spec.baseline)
            self.assertEqual(spec.variant_names(), ["variant_a"])
        finally:
            os.unlink(tmp_path)

    def test_experiment_config_rejects_unknown_baseline(self):
        task_cfg = os.path.join(ROOT, "configs/task/pusht_smoke_dataset.yaml")
        planner_cfg = os.path.join(ROOT, "configs/planner/smoke.yaml")
        backend_cfg = os.path.join(ROOT, "configs/backend/gt_env_state.yaml")
        fidelity_cfg = os.path.join(ROOT, "configs/fidelity/finest.yaml")
...
"""
    ...
    def test_aggregate_summary_returns_pairwise_rows_when_baseline_is_provided(self):
        rows = [
            {
                "variant_name": "baseline",
                "rollout_id": "r0",
        ...
        self.assertEqual(paired_rows[0]["reference_variant"], "baseline")
        self.assertEqual(paired_rows[0]["variant_name"], "challenger")
        self.assertEqual(paired_rows[0]["rollout_id"], "r0")
        self.assertEqual(paired_rows[0]["success_delta"], -1)
        self.assertAlmostEqual(paired_rows[0]["final_pos_diff_delta"], 2.0)
        self.assertAlmostEqual(paired_rows[0]["plan_time_total_sec_delta"], 1.0)
